...
import type { EmployeeDocument, DocumentType } from '@/types/database'
...
export default function EmployeeDocumentsTab({
  ...
  const loadDocuments = useCallback(async () => {
    setLoading(true)
    setError(null)
    const { data, error: fetchError } = await getDocuments('employee', employeeId)
    setLoading(false)
    if (fetchError) {
      setError(fetchError)
    } else {
  ...
  async function openAddForm() {
    ...
    if (documentTypes.length === 0) {
      const { data } = await getDocumentTypes('employee')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: created, error: createError } = await createDocument('employee', employeeId, fd)
    ...
    for (const file of pendingFiles) {
      const { error: uploadError } = await uploadDocumentFile(created.id, 'employee', file)
      if (uploadError) {
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-2 pt-4">
              ...
              />
              <label htmlFor="sin-vencimiento-new" className="text-xs text-gray-600">
                Sin vencimiento
              </label>
            </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="mt-1 text-xs text-gray-400">
            Los documentos del empleado aparecerán aquí una vez que se agreguen.
          </p>
        </div>
      ) : (
